            ...
            <nav className="border-b border-[#eeeeee] pb-4">
              <ul className="list-none p-0 m-0 font-medium text-sm flex gap-1 w-full">
                {tabs.map((item) => (
                  ...
                  >
                    <motion.span className="flex flex-nowrap">
                      {item.icon}
                    </motion.span>
                    <motion.span className="flex flex-nowrap text-nowrap">
                      {item.label}
                    </motion.span>
                    {item === selectedTab ? (
                      <motion.div
              ...
              <AnimatePresence mode="wait">
                <motion.div
                  ...
                  className={cn(
                    ...
                    selectedTab.label === "Pack view" && "relative"
                  )}
                  layout
                  transition={{ duration: 0.3, layout: { type: "spring", stiffness: 100, damping: 15 } }}
                >
                  {bounties.map((item, index) => (
                    <motion.div
                      ...
                      className={cn(
                        "border p-2 rounded bg-white shadow",
                        selectedTab.label === "List view" &&
                      ...
                      style={
                        selectedTab.label === "Pack view"
                          ? {
                              rotate: `${
                                index * 5 - (bounties.length - 1) * 2.5
                              }deg`, // Slight rotation
                              zIndex: bounties.length - index, // Higher index goes on top
                              scale: 1 - index * 0.05, // Slightly smaller for cards behind
                    ...
                    >
                      <motion.div>
                        <motion.img
                          ...
                          height={
                            selectedTab.label === "List view" ? "48" : "148"
                          }
                          alt="wanted posters"
                        />
                      </motion.div>
                      <motion.div className="space-2">
                        ...
                        <motion.p className="text-base text-gray-800">
                          ...
                          >
                            ...
                            />{" "}
                            <motion.span layoutId={`price-${item.name}`}>
                              {item.price}
                            </motion.span>
                          </motion.span>{" "}
                        </motion.p>
